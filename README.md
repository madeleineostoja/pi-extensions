...
| [pi-implement](packages/pi-implement)         | Execute Markdown plans through checkout-owned workstreams with cumulative review, durable recovery, and serialized publication. |
...
`pi-implement` requires Node.js 24+ and scopes each active run to its invoking checkout. Separate linked worktrees may run concurrently; a second run in the same checkout is rejected by its OS-backed lease. Workstream concurrency controls implementation overlap, while staging and publication remain serialized. User or third-party mutation of the active target checkout, branch, or pi-owned workspace pauses the run when safety cannot be proved; pi-implement never removes Git lock files. Its revisioned `run-state.json` is authoritative, while events, UI, evidence, and plan checkboxes are projections. Historical runs cannot resume. See the [pi-implement README](packages/pi-implement/README.md) for operating constraints.
