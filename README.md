...
`pi-implement` requires Node.js 24+ and scopes each active run to its invoking checkout. It accepts one arbitrary or headingless Markdown checkbox section and recursively ingests only ordinary local Markdown links as plan corpus. Separate linked worktrees may run concurrently; a second run in the same checkout is rejected by its OS-backed lease. Workstream concurrency controls implementation overlap, while staging and publication remain serialized. A failed target, branch, or protected-artifact safety check prevents that publication attempt; pi-implement never removes Git lock files. Its revisioned `run-state.json` is authoritative, while UI, evidence, and plan checkboxes are projections. Historical runs cannot resume. See the [pi-implement README](packages/pi-implement/README.md) for operating constraints and the [shared library README](lib/README.md) for the lease and common-Git exclude contracts.
