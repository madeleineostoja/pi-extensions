...
const GUARD_TOOLS_FALLBACK_MSG =
  "--guard-tools value was empty after parsing; falling back to default (edit,write).";

export default function (pi: ExtensionAPI) {
  let guardMode = true;
  let nonInteractiveNotified = false;
  let triggerTools = new Set<string>(["edit", "write"]);
  ...
  pi.registerFlag("guard-tools", {
    type: "string",
    description: "Comma-separated list of tool names to guard (default: edit,write). Replaces the default set entirely.",
  });
  ...
  pi.on("session_start", async (event, ctx) => {
    const FRESH_START_REASONS = new Set<typeof event.reason>(["startup", "new", "fork"]);
    if (FRESH_START_REASONS.has(event.reason)) {
      applyMode(true);
    }
    nonInteractiveNotified = false;

    const flagValue = pi.getFlag("guard-tools");
    const { tools, usedDefault } = parseGuardTools(typeof flagValue === "string" ? flagValue : undefined);
    triggerTools = tools;
    if (usedDefault) {
      process.stderr.write(`[pi-guard] ${GUARD_TOOLS_FALLBACK_MSG}\n`);
      ctx.ui.notify(GUARD_TOOLS_FALLBACK_MSG, "warning");
    }
    ...
    handler: async (args, ctx) => {
      ...
      if (action.kind === "toggle") {
        ...
        if (action.value === guardMode) {
          ctx.ui.notify(`guard mode: already ${guardMode ? "on" : "off"}`, "info");
          return;
        }
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    try {
      ...
      if (choice === "Steer") {
        message = (await ctx.ui.input(formatSteerTitle(toolPath), "what should the agent do differently?")) ?? "";
      }
    } catch (err) {
