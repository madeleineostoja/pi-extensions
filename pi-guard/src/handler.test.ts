...
const DECLINED_NO_FEEDBACK = "Edit not applied. User declined without feedback. Ask for clarification before retrying.";

function makeToolCallCtx(overrides: Partial<{
  ...
  return {
    ...
    signal: undefined,
    ui: {
      select: typeof overrides.selectResult === "function"
        ? overrides.selectResult
        : () => Promise.resolve(overrides.selectResult as string | undefined),
      input: typeof overrides.inputResult === "function"
...
function captureToolCallHandler() {
  let toolCallHandler: ((event: ToolCallEvent, ctx: ExtensionContext) => Promise<unknown>) | undefined;
...
function makeEditEvent(input: Record<string, unknown> = {}): ToolCallEvent {
  return { type: "tool_call", toolCallId: "test-1", toolName: "edit", input: input as never } as ToolCallEvent;
}
...
describe("decideToolCall", () => {
  describe("guardMode=false", () => {
    it("passes when guard is off, tool in set, has UI", () => {
      expect(
        decideToolCall({ guardMode: false, hasUI: true, toolName: "edit", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("pass");
    });

    it("passes when guard is off, tool in set, no UI", () => {
      expect(
        decideToolCall({ guardMode: false, hasUI: false, toolName: "edit", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("pass");
    });

    it("passes when guard is off, tool not in set, has UI", () => {
      expect(
        decideToolCall({ guardMode: false, hasUI: true, toolName: "bash", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("pass");
    });

    it("passes when guard is off, tool not in set, no UI", () => {
      expect(
        decideToolCall({ guardMode: false, hasUI: false, toolName: "bash", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("pass");
    });
  });
  ...
  describe("guardMode=true, tool NOT in trigger set", () => {
    it("passes when tool is not in set, has UI", () => {
      expect(
        decideToolCall({ guardMode: true, hasUI: true, toolName: "bash", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("pass");
    });

    it("passes when tool is not in set, no UI", () => {
      expect(
        decideToolCall({ guardMode: true, hasUI: false, toolName: "bash", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("pass");
    });
  });
  ...
  describe("guardMode=true, tool IN trigger set", () => {
    it("prompts when tool is 'edit' and has UI", () => {
      expect(
        decideToolCall({ guardMode: true, hasUI: true, toolName: "edit", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("prompt");
    });

    it("prompts when tool is 'write' and has UI", () => {
      expect(
        decideToolCall({ guardMode: true, hasUI: true, toolName: "write", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("prompt");
    });

    it("auto-disables when tool is 'edit' and no UI", () => {
      expect(
        decideToolCall({ guardMode: true, hasUI: false, toolName: "edit", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("auto-disable");
    });

    it("auto-disables when tool is 'write' and no UI", () => {
      expect(
        decideToolCall({ guardMode: true, hasUI: false, toolName: "write", triggerTools: TRIGGER_TOOLS, nonInteractiveAlreadyNotified: false })
      ).toBe("auto-disable");
    });
  });
  ...
  it("passes for a tool that is not in an empty trigger set", () => {
    expect(
      decideToolCall({ guardMode: true, hasUI: true, toolName: "edit", triggerTools: new Set(), nonInteractiveAlreadyNotified: false })
    ).toBe("pass");
  });
});
...
describe("resolveChoice", () => {
  it('"Accept" returns not blocked, no side effect', () => {
    expect(resolveChoice({ choice: "Accept", message: undefined })).toEqual({ block: false });
  });

  it('"Accept and stop guarding" returns not blocked with disable side effect', () => {
    expect(resolveChoice({ choice: "Accept and stop guarding", message: undefined })).toEqual({
      block: false,
      sideEffect: "disable",
    });
  });
  ...
  it('"Steer" with a message returns blocked with formatted reason', () => {
    const result = resolveChoice({ choice: "Steer", message: "use a class not a function" });
  ...
  it("unexpected string returns blocked with declined-without-feedback reason", () => {
    const result = resolveChoice({ choice: "some-unknown-value", message: undefined });
    expect(result.block).toBe(true);
    expect(result.reason).toBe(
      "Edit not applied. User declined without feedback. Ask for clarification before retrying."
    );
...
function makeCapturingCtx(selectResult: string | undefined): ExtensionContext & {
  ...
  return {
    ...
    signal: undefined,
    ui: {
      select: (title: string) => { selectCalls.push({ title }); return Promise.resolve(selectResult); },
      input: (title: string) => { inputCalls.push({ title }); return Promise.resolve(undefined); },
      setStatus: () => {},
      notify: () => {},
      confirm: () => Promise.resolve(false),
      onTerminalInput: () => () => {},
...
describe("tool_call handler (integration)", () => {
  it("Steer chosen + input returns undefined → blocking result with declined-without-feedback reason", async () => {
    const handler = captureToolCallHandler();
    const ctx = makeToolCallCtx({ selectResult: "Steer", inputResult: undefined });
...
function makeInteractiveCtx(): ExtensionContext & { statusCalls: Array<{ key: string; value: string | undefined }> } {
...
const FOOTER_TEXT = "📝 guarding edits";
...
describe("session_start reason handling", () => {
  it('reason "startup" enables guard and sets footer', async () => {
    ...
    await sessionStartHandler({ type: "session_start", reason: "startup" } as never, ctx);
  ...
  it('reason "new" enables guard and sets footer', async () => {
    ...
    await sessionStartHandler({ type: "session_start", reason: "new" } as never, ctx);
  ...
  it('reason "fork" enables guard and sets footer', async () => {
    ...
    await sessionStartHandler({ type: "session_start", reason: "fork" } as never, ctx);
  ...
  it('reason "reload" does not call applyMode(true): guard remains off if previously disabled', async () => {
    const { sessionStartHandler, toolCallHandler } = captureHandlers();
    const acceptCtx = makeToolCallCtx({ selectResult: "Accept and stop guarding" });

    await toolCallHandler(makeEditEvent() as never, acceptCtx);

    const reloadCtx = makeInteractiveCtx();
    await sessionStartHandler({ type: "session_start", reason: "reload" } as never, reloadCtx);
  ...
  it('reason "resume" does not call applyMode(true): guard remains off if previously disabled', async () => {
    ...
    await sessionStartHandler({ type: "session_start", reason: "resume" } as never, resumeCtx);
...
function captureHandlersWithFlag(guardToolsFlag: string | undefined) {
  let toolCallHandler: AnyHandler | undefined;
  let sessionStartHandler: AnyHandler | undefined;
  ...
  const pi = {
    ...
    registerTool: () => {},
    registerFlag: () => {},
    getFlag: (name: string) => name === "guard-tools" ? guardToolsFlag : undefined,
    registerMessageRenderer: () => {},
    sendMessage: () => {},
    sendUserMessage: () => {},
    appendEntry: () => {},
...
function makeToolCallEventFor(toolName: string): ToolCallEvent {
  return { type: "tool_call", toolCallId: "test-custom", toolName, input: {} as never } as ToolCallEvent;
}

describe("configurable guard tools (--guard-tools flag)", () => {
  it("custom tool name triggers modal when included in --guard-tools", async () => {
    const { sessionStartHandler, toolCallHandler } = captureHandlersWithFlag("edit,write,custom-edit");
    const interactiveCtx = makeInteractiveCtx();
    await sessionStartHandler({ type: "session_start", reason: "startup" } as never, interactiveCtx);

    const capturingCtx = makeCapturingCtx("Accept");
    const result = await toolCallHandler(makeToolCallEventFor("custom-edit") as never, capturingCtx);

    expect(capturingCtx.selectCalls).toHaveLength(1);
    expect(result).toBeUndefined();
  });

  it("write is no longer guarded when --guard-tools=edit only", async () => {
    const { sessionStartHandler, toolCallHandler } = captureHandlersWithFlag("edit");
    const interactiveCtx = makeInteractiveCtx();
    await sessionStartHandler({ type: "session_start", reason: "startup" } as never, interactiveCtx);

    const capturingCtx = makeCapturingCtx("Accept");
    const result = await toolCallHandler(makeToolCallEventFor("write") as never, capturingCtx);

    expect(capturingCtx.selectCalls).toHaveLength(0);
    expect(result).toBeUndefined();
  });

  it("empty --guard-tools falls back to default and emits warning notification", async () => {
    const { sessionStartHandler, toolCallHandler } = captureHandlersWithFlag("");
    ...
    const ctx: ExtensionContext = {
      ...
      ui: {
        ...
        input: () => Promise.resolve(undefined),
        setStatus: () => {},
        notify: (message: string, level?: "info" | "warning" | "error") => { notifyCalls.push({ message, level: level ?? "info" }); },
        confirm: () => Promise.resolve(false),
        onTerminalInput: () => () => {},
        setWorkingMessage: () => {},
        setWorkingVisible: () => {},
    ...
    const warningNotification = notifyCalls.find((n) => n.level === "warning");
    expect(warningNotification).toBeDefined();
    expect(warningNotification?.message).toMatch(/falling back to default/);

    const capturingCtx = makeCapturingCtx("Accept");
    await toolCallHandler(makeToolCallEventFor("edit") as never, capturingCtx);
    expect(capturingCtx.selectCalls).toHaveLength(1);

    const capturingCtx2 = makeCapturingCtx("Accept");
    await toolCallHandler(makeToolCallEventFor("write") as never, capturingCtx2);
    expect(capturingCtx2.selectCalls).toHaveLength(1);
  });
});
...
function captureCommandAndShortcutHandlers() {
  ...
  if (!commandHandler) throw new Error("guard command handler was not registered");
  if (!shortcutHandler) throw new Error("ctrl+shift+g shortcut handler was not registered");
  return { commandHandler, shortcutHandler };
}

function makeNotifyCapturingCtx(guardOn: boolean = true): ExtensionContext & { notifyCalls: Array<{ message: string; level: string }> } {
  const notifyCalls: Array<{ message: string; level: string }> = [];
  return {
    hasUI: true,
    signal: undefined,
