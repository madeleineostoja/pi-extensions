...
On **Windows**, **Alpine Linux** (musl), **distroless**, and **BSDs**, `nono` is not available. The sandbox will run in in-process-only mode with a one-time warning — Claude Code still loads and the in-process tool gate remains active, but subprocesses run unsandboxed.
...
When set to `true` in the policy config, `sandboxExtension` will refuse to start if the `nono` kernel sandbox binary is unavailable (i.e. `getBinaryStatus()` returns anything other than `{ kind: "ok" }`). An error notification is emitted and no `tool_call` handler is registered, leaving the Pi instance untouched.
...
Agent tool call
      ...
      ▼
┌─────────────────────┐
│  sandbox tool gate  │  (in-process, FS + exec)
│  block → abort      │
└────────┬────────────┘
         │ allowed
         ▼
...
```ts
import type { SandboxAuditEvent, SandboxPolicyChangedEvent } from "@earendil-works/pi-sandbox";
```

## Threat model

The in-process tool gate intercepts Pi's built-in filesystem tools (`read`, `write`, `edit`, `ls`, `find`, `grep`). Two deliberate limitations are worth noting:
