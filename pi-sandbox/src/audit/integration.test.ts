...
describe("stub listener receives sandbox:audit events with documented shape", () => {
  const kindDecisionPairs: Array<{
    ...
    { kind: "fs", decision: "allowed", extra: { path: "/home/user/project/src/index.ts", tool: "read" } },
    { kind: "fs", decision: "blocked", extra: { path: "/etc/passwd", tool: "read", rule: "allowList:read" } },
    { kind: "network", decision: "allowed", extra: { host: "api.github.com" } },
    { kind: "network", decision: "blocked", extra: { host: "evil.example.com" } },
    { kind: "exec", decision: "allowed", extra: { tool: "bash" } },
    { kind: "exec", decision: "blocked", extra: { tool: "bash", rule: "sandboxOff=false" } },
    { kind: "policy-change", decision: "granted", extra: { scope: "session", source: "command" } },
    { kind: "policy-change", decision: "revoked", extra: { scope: "persisted", source: "command" } },
  ];
  ...
  it("payload does not contain unexpected enumerable keys beyond the schema", () => {
    const { target, received } = makeStubListener();
    recordAudit({ kind: "fs", decision: "blocked", path: "/tmp/x", tool: "read" }, { events: target });

    const payload = received[0].payload as SandboxAuditEvent;
    const allowedKeys = new Set(["ts", "kind", "decision", "rule", "tool", "toolCallId", "path", "host", "scope", "source"]);
...
describe("policy-change events — both event types emitted", () => {
  it("granted: listener receives sandbox:audit and sandbox:policy-changed", () => {
    ...
    const auditEvents = received.filter((e) => e.event === "sandbox:audit");
    const policyEvents = received.filter((e) => e.event === "sandbox:policy-changed");
  ...
  it("revoked: listener receives sandbox:policy-changed with correct scope", () => {
    const { target, received } = makeStubListener();
    recordAudit(
      { kind: "policy-change", decision: "revoked", scope: "persisted", source: "command" },
      { events: target }
    );

    const policyPayload = received.find((e) => e.event === "sandbox:policy-changed")!.payload as SandboxPolicyChangedEvent;
...
describe("external sandbox:audit emission does not mutate sandbox state", () => {
  ...
  it("a rogue sandbox:audit event cannot append to the ring buffer", () => {
    ...
    emitter.emitAuditEvents(
      { ts: Date.now(), kind: "network", decision: "blocked", host: "real.example.com" } as SandboxAuditEvent,
      target
    );
  ...
  it("emitting sandbox:policy-changed from outside has no effect on session state", () => {
    ...
    const cmds = createSlashCommands({ recordAudit: ra, getRecentBlockedHosts });
