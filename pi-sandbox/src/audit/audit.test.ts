...
import type { RecordAuditOptions, AuditPipeline } from "./audit.js";
...
describe("entry schema — kind × decision combinations", () => {
  ...
  it("fs entry carries path when provided", () => {
    const { target: t, emitted: e } = makeEvents();
    recordAudit({ kind: "fs", decision: "blocked", path: "/secret/.env", tool: "read" }, { events: t });
  ...
  it("network entry carries host when provided", () => {
    const { target: t, emitted: e } = makeEvents();
    recordAudit({ kind: "network", decision: "blocked", host: "evil.com" }, { events: t });
    const payload = e[0].payload as SandboxAuditEvent;
  ...
  it("policy-change entry carries scope and source", () => {
    const { target: t, emitted: e } = makeEvents();
    recordAudit(
      { kind: "policy-change", decision: "granted", scope: "session", source: "command" },
      { events: t }
    );
    const payload = e[0].payload as SandboxAuditEvent;
...
describe("sandbox:policy-changed event", () => {
  it("emits sandbox:policy-changed for policy-change kind entries", () => {
    const { target, emitted } = makeEvents();
    recordAudit(
      { kind: "policy-change", decision: "granted", scope: "persisted", source: "command" },
      { events: target }
    );

    const policyChangedEvents = emitted.filter((x) => x.event === "sandbox:policy-changed");
  ...
  it("emits both sandbox:audit and sandbox:policy-changed for policy-change entries", () => {
    const { target, emitted } = makeEvents();
    recordAudit({ kind: "policy-change", decision: "revoked" }, { events: target });

    expect(emitted.filter((x) => x.event === "sandbox:audit")).toHaveLength(1);
    expect(emitted.filter((x) => x.event === "sandbox:policy-changed")).toHaveLength(1);
  });
});
...
describe("JSONL file writer", () => {
  ...
  it("creates the log file and writes a valid JSON line", () => {
    ...
    recordAudit({ kind: "fs", decision: "blocked", path: "/etc/passwd" }, { logFile });
  ...
  it("appends multiple entries, one per line", () => {
    ...
    recordAudit({ kind: "network", decision: "blocked", host: "bad.com" }, { logFile });
  ...
  it("does not write to disk when logEnabled is false", () => {
    const logFile = path.join(tmpDir, "sandbox-audit.jsonl");
    recordAudit({ kind: "fs", decision: "blocked" }, { logFile, logEnabled: false });
  ...
  it("still emits events when logEnabled is false", () => {
    const logFile = path.join(tmpDir, "sandbox-audit.jsonl");
    const { target, emitted } = makeEvents();
    recordAudit({ kind: "fs", decision: "blocked" }, { logFile, logEnabled: false, events: target });
...
describe("log rotation", () => {
  ...
  it("rotates the log file when it exceeds maxBytes", () => {
    ...
    for (let i = 0; i < 20; i++) {
      recordAudit({ kind: "fs", decision: "blocked", path: `/a/very/long/path/number/${i}` }, { logFile, maxBytes });
    }
  ...
  it("preserves the previous file as .1 after rotation", () => {
    ...
    for (let i = 0; i < 10; i++) {
      recordAudit({ kind: "exec", decision: "allowed", tool: `tool-${i}` }, { logFile, maxBytes });
    }
  ...
  it("respects maxFiles by not keeping more than N rotated files", () => {
    ...
    for (let i = 0; i < 50; i++) {
      recordAudit({ kind: "fs", decision: "blocked", path: `/path/${i}` }, { logFile, maxBytes, maxFiles });
    }
...
describe("permission-failure fallback", () => {
  ...
  it("does not throw when log directory cannot be created", () => {
    ...
    expect(() =>
      logWriter.appendLogEntry(
        { ts: Date.now(), kind: "fs", decision: "blocked" } as import("./schema.js").AuditEntry,
        { logFile }
      )
    ).not.toThrow();
  });
  ...
  it("still emits events even when the log directory is unwritable", () => {
    ...
    recordAudit({ kind: "fs", decision: "blocked" }, { logFile, events: target });
...
describe("recently-blocked hosts ring buffer", () => {
  ...
  function makeFullEntry(partial: Parameters<typeof recordAudit>[0]): import("./schema.js").AuditEntry {
  ...
  it("caps the ring buffer at 20 entries", () => {
    ...
    for (let i = 0; i < 25; i++) {
      emitter.emitAuditEvents(
        makeFullEntry({ kind: "network", decision: "blocked", host: `host${i}.com` }),
        target
      );
    }
...
describe("toolGate onAudit hook", () => {
  it("calls onAudit with a blocked fs entry when denyPattern fires", async () => {
    const { createToolGate, BLOCK_REASON } = await import("../enforcement/toolGate.js");
    ...
    try {
      gate = createToolGate({
        getPolicy: () => ({
          enabled: true,
          fs: { allowRead: [tmpDir], allowWrite: [], denyPatterns: ["**/.env"] },
        ...
        subscribe: () => () => {},
        onAudit: (entry) => auditCalls.push(entry as Omit<SandboxAuditEvent, "ts">),
      });

      const envFile = path.join(tmpDir, ".env");
      fs.writeFileSync(envFile, "SECRET=x");
      const result = await gate.handleToolCall({ type: "tool_call", toolCallId: "test", toolName: "read", input: { path: envFile } } as Parameters<typeof gate.handleToolCall>[0]);
  ...
  it("calls onAudit with a blocked fs entry when allowList miss occurs", async () => {
    ...
    try {
      ...
      await gate.handleToolCall({ type: "tool_call", toolCallId: "test", toolName: "read", input: { path: file } } as Parameters<typeof gate.handleToolCall>[0]);
