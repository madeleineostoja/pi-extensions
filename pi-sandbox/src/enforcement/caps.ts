...
export function createCaps(): CapsInstance {
  ...
  function buildManifest(policy: Policy, ctx: ManifestContext): CapabilityManifest {
    const platform = ctx.platform ?? process.platform;
    const cwd = ctx.cwd;
    const manifest: CapabilityManifest = {};

    const allowRead = filterExistingPaths(policy.fs.allowRead, cwd, ctx, warnedMissingPaths);
    const allowWrite = filterExistingPaths(policy.fs.allowWrite, cwd, ctx, warnedMissingPaths);
  ...
  function buildCapabilitySet(policy: Policy, ctx: ManifestContext): CapabilitySet {
    const platform = ctx.platform ?? process.platform;
    const cwd = ctx.cwd;

    const allowRead = new Set(filterExistingPaths(policy.fs.allowRead, cwd, ctx, warnedMissingPaths));
    const allowWrite = new Set(filterExistingPaths(policy.fs.allowWrite, cwd, ctx, warnedMissingPaths));
    ...
    if (platform === "darwin") {
      for (const pattern of policy.fs.denyPatterns) {
        const prefix = literalPrefix(pattern);
        if (prefix !== null) {
          platformRules.push(`(deny file-read* (path-prefix "${escapeSeatbeltString(prefix)}"))`);
        }
      }
    }
