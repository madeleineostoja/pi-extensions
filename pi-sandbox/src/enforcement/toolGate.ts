 ...
 *
 * TODO: Wire pi.on("tool_call", handler) once @earendil-works/pi-coding-agent
 * is installable. Until then, createToolGate() returns a handler the caller
 * wires manually.
 */
...
export function createToolGate(opts: ToolGateOptions): ToolGate {
  ...
  async function handleToolCall(event: ToolCallEvent): Promise<ToolGateResult | undefined> {
    ...
    const modes: AccessMode[] = spec.mode === "readwrite" ? ["read", "write"] : [spec.mode];
    for (const mode of modes) {
      const decision = await decideFsAccess(rawPath, mode, eff, { cwd: ctx.cwd });
      if (!decision.allow) {
        onAudit?.({
          kind: "fs",
          decision: "blocked",
