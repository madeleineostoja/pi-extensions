...
function makeEvent(toolName: string, input: Record<string, unknown>): ToolCallEvent {
  return { type: "tool_call", toolCallId: "test", toolName, input } as ToolCallEvent;
}
...
function makeGate(policy: Policy, cwd: string, extraOpts: Partial<ToolGateOptions> = {}): ToolGate {
  let current = policy;
...
describe("resolveAbsolute — path resolution", () => {
  ...
  it("blocks a read of an existing file outside allowRead", async () => {
    ...
    try {
      const result = await deniedGate.handleToolCall(makeEvent("read", { path: file }));
  ...
  it("handles a relative path by resolving against cwd", async () => {
    ...
    fs.writeFileSync(file, "hi");
    const result = await gate.handleToolCall(makeEvent("read", { path: "rel.txt" }));
    expect(result).toBeUndefined();
  });

  it("handles path traversal (../../etc/passwd relative to cwd)", async () => {
    const result = await gate.handleToolCall(makeEvent("read", { path: "../../etc/passwd" }));
  ...
  it("resolves a symlink pointing to a denied path and blocks it", async () => {
    ...
    try {
      const result = await symlinkGate.handleToolCall(makeEvent("read", { path: symlink }));
      expect(result).toEqual({ block: true, reason: BLOCK_REASON });
    } finally {
      symlinkGate.dispose();
    }
  ...
  it("allows a write to a non-existent file in an allowed parent dir (missing leaf)", async () => {
    ...
    const result = await gate.handleToolCall(makeEvent("write", { path: newFile }));
    expect(result).toBeUndefined();
  });
});
...
describe("FS tools — allowed paths", () => {
  ...
  it.each(["read", "ls", "find", "grep"] as const)(
    ...
    async (tool) => {
      ...
      try {
        const result = await gate.handleToolCall(makeEvent(tool, { path: file }));
        expect(result).toBeUndefined();
      } finally {
        gate.dispose();
      }
  ...
  it("write: allowed path → no block", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("write", { path: path.join(tmpDir, "out.txt") }));
      expect(result).toBeUndefined();
    } finally {
      gate.dispose();
    }
  ...
  it("edit: path in both allowRead and allowWrite → no block", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("edit", { path: file }));
      expect(result).toBeUndefined();
    } finally {
      gate.dispose();
    }
...
describe("FS tools — denied paths", () => {
  ...
  it.each(["read", "ls", "find", "grep"] as const)(
    ...
    async (tool) => {
      ...
      const gate = makeGate(policy, tmpDir);
      try {
        const result = await gate.handleToolCall(makeEvent(tool, { path: outsidePath }));
        expect(result).toEqual({ block: true, reason: BLOCK_REASON });
      } finally {
        gate.dispose();
      }
  ...
  it("write: path outside allowWrite → block", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("write", { path: "/etc/hosts" }));
      expect(result).toEqual({ block: true, reason: BLOCK_REASON });
    } finally {
      gate.dispose();
    }
...
describe("denyPatterns glob semantics", () => {
  ...
  it("**/.env matches .env in the cwd subtree", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("read", { path: envFile }));
      expect(result).toEqual({ block: true, reason: BLOCK_REASON });
    } finally {
      gate.dispose();
    }
  ...
  it("**/.env blocks a write to .env even when write is in allowWrite", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("write", { path: envFile }));
      expect(result).toEqual({ block: true, reason: BLOCK_REASON });
    } finally {
      gate.dispose();
    }
  ...
  it("**/.env.* matches .env.local", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("read", { path: envLocal }));
      expect(result).toEqual({ block: true, reason: BLOCK_REASON });
    } finally {
      gate.dispose();
    }
  ...
  it("denyPatterns are authoritative on Linux (glob deny independent of platform)", async () => {
    ...
    try {
      const result = await gate.handleToolCall(makeEvent("read", { path: pemFile }));
      expect(result).toEqual({ block: true, reason: BLOCK_REASON });
    } finally {
      gate.dispose();
    }
...
describe("block reason uniformity", () => {
  ...
  it("denyPattern block returns the generic reason", async () => {
    ...
    fs.writeFileSync(file, "x");
    const policy = makePolicy({ allowRead: [tmpDir], denyPatterns: ["**/.env"] });
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("read", { path: file }));
  ...
  it("allowRead miss returns the generic reason", async () => {
    ...
    const gate = makeGate(policy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("read", { path: "/etc/hosts" }));
      expect(result?.reason).toBe(BLOCK_REASON);
...
describe("policy.enabled === false", () => {
  ...
  it("allows write matching denyPattern when policy is disabled", async () => {
    ...
    const gate = makeGate(disabledPolicy, tmpDir);
    try {
      const result = await gate.handleToolCall(makeEvent("write", { path: path.join(tmpDir, ".env") }));
      expect(result).toBeUndefined();
    } finally {
      gate.dispose();
    }
...
describe("bash tool skip", () => {
  it("bash events are always passed through (kernel handles them)", async () => {
    const policy = makePolicy({ allowRead: [], allowWrite: [], denyPatterns: ["**/*"] });
    const gate = makeGate(policy, os.tmpdir());
    try {
      const result = await gate.handleToolCall(makeEvent("bash", { command: "cat /etc/passwd" }));
      expect(result).toBeUndefined();
    } finally {
      gate.dispose();
    }
...
describe("unknown tools", () => {
  it("unknown tool events are passed through", async () => {
    ...
    const gate = makeGate(policy, os.tmpdir());
    try {
      const result = await gate.handleToolCall(makeEvent("some_unknown_tool", { path: "/etc/passwd" }));
      expect(result).toBeUndefined();
    } finally {
      gate.dispose();
    }
...
describe("policy reload", () => {
  ...
  it("a path allowed after reload is no longer blocked", async () => {
    ...
    try {
      const before = await gate.handleToolCall(makeEvent("read", { path: file }));
      ...
      const after = await gate.handleToolCall(makeEvent("read", { path: file }));
  ...
  it("a path denied after reload is now blocked", async () => {
    ...
    try {
      ...
      currentPolicy = makePolicy({ allowRead: [tmpDir], denyPatterns: ["**/*.key"] });
      for (const fn of subscribers) fn(currentPolicy);
