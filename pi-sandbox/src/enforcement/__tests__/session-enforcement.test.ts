...
describe("session enforcement — /sandbox off", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = makeTmpDir();
    const policy = makePolicy({ fs: { allowRead: [], allowWrite: [], denyPatterns: ["**/.env", "**/.ssh/**"] } });
...
describe("session enforcement — createSpawnHook honors /sandbox off", () => {
  let tmpDir: string;
  let cmds: ReturnType<typeof createSlashCommands>;
  ...
  it("passes command through unwrapped when sandbox is off", () => {
    const ctx = makeSubcommandCtx(tmpDir);
    cmds.dispatch("off", ctx);
    ...
    const input = { command: "cat /etc/passwd", cwd: tmpDir };
  ...
  it("emits audit entry with rule session:sandbox-off when sandbox is off", () => {
    ...
    hook({ command: "cat /etc/passwd", cwd: tmpDir });
  ...
  it("wraps command when sandbox is on", () => {
    const policy = makePolicy();
    const nonoPath = "/usr/local/bin/nono";
    const hook = createSpawnHook(
      () => policy,
    ...
    expect(output.command).toContain("nono");
    expect(output.command).not.toBe(input.command);
  });

  it("passes command through again after dispatch('on')", () => {
    const ctx = makeSubcommandCtx(tmpDir);
    cmds.dispatch("off", ctx);
    cmds.dispatch("on", ctx);
...
describe("session enforcement — createUserBashHandler honors /sandbox off", () => {
  ...
  it("passes command through unwrapped when sandbox is off", () => {
    ...
    const event = { command: "cat /etc/passwd", cwd: tmpDir };
    const result = handler(event);

    expect(result.operations).toHaveLength(1);
    expect(result.operations[0].command).toBe(event.command);
  });
  ...
  it("emits audit entry with rule session:sandbox-off when sandbox is off", () => {
    ...
    handler({ command: "cat /etc/passwd", cwd: tmpDir });
  ...
  it("wraps command when sandbox is on", () => {
    ...
    expect(result.operations).toHaveLength(1);
    expect(result.operations[0].command).toContain("nono");
    expect(result.operations[0].command).not.toBe(event.command);
  });

  it("wraps command again after dispatch('on')", () => {
    const ctx = makeSubcommandCtx(tmpDir);
    cmds.dispatch("off", ctx);
    cmds.dispatch("on", ctx);
