...
function validateAgainstSchema(manifest: unknown): { valid: boolean; errors: string[] } {
  const valid = validateSchema(manifest) as boolean;
  const errors = valid
    ? []
    : ((validateSchema.errors ?? []) as Array<{ instancePath: string; message?: string }>).map(
  ...
  it("manifest with all fields validates against the schema", () => {
    ...
    try {
      const policy = makeBasePolicy({
        fs: { allowRead: [tmpDir], allowWrite: [tmpDir], denyPatterns: ["/etc/passwd"] },
        network: { mode: "always", allow: ["example.com", "*.example.com"] },
      });
      const manifest = caps.buildManifest(policy, makeCtx({ platform: "darwin" }));
      const { valid, errors } = validateAgainstSchema(manifest);
      expect(errors).toEqual([]);
      expect(valid).toBe(true);
    } finally {
...
describe("buildManifest — network.mode × hasUI", () => {
  ...
  it('mode "off" → emits deny-all network section (allow_domain present and empty)', () => {
    const policy = makeBasePolicy({ network: { mode: "off", allow: ["example.com"] } });
  ...
  it('mode "always" + hasUI=false → strict allowlist', () => {
    const policy = makeBasePolicy({ network: { mode: "always", allow: ["example.com"] } });
    const manifest = caps.buildManifest(policy, makeCtx({ hasUI: false }));
  ...
  it('empty allowlist + mode "non-interactive-only" + hasUI=false → deny-all network shape', () => {
    const policy = makeBasePolicy({ network: { mode: "non-interactive-only", allow: [] } });
    const manifest = caps.buildManifest(policy, makeCtx({ hasUI: false }));
    expect(manifest.network).toBeDefined();
    expect(manifest.network?.allow_domain).toEqual([]);
  });
...
describe("buildManifest — networkOff session override produces deny-all", () => {
  ...
  it("applySessionOverrides with networkOff=true + buildManifest produces { allow_domain: [] }", () => {
    const basePolicy = makeBasePolicy({ network: { mode: "non-interactive-only", allow: ["github.com"] } });
...
describe("buildManifest — missing path filtering", () => {
  ...
  it("existing paths are included in allow_read", () => {
    const tmpDir = makeTmpDir();
    try {
      const policy = makeBasePolicy({ fs: { allowRead: [tmpDir], allowWrite: [], denyPatterns: [] } });
      const ctx = makeCtx();
      const manifest = caps.buildManifest(policy, ctx);
      // realpathSync canonicalizes symlinks (e.g. /tmp → /private/tmp on macOS)
      expect(manifest.filesystem?.allow_read).toContain(fs.realpathSync(tmpDir));
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("missing path warning is emitted only once (one-time warning)", () => {
    ...
    caps.buildManifest(policy, ctx);
    const notifyCalls = (ctx.ui.notify as ReturnType<typeof vi.fn>).mock.calls.filter(
      ([msg]) => (msg as string).includes(missingPath)
    );
...
describe("buildManifest — macOS deny translation", () => {
  ...
  it("Linux: no filesystem.deny entries regardless of denyPatterns", () => {
    const policy = makeBasePolicy({
      fs: { allowRead: [], allowWrite: [], denyPatterns: ["/etc/passwd", "**/*.pem"] },
    });
...
describe("buildCapabilitySet", () => {
  ...
  it("Linux: allows-only, queryPath returns true for allowed read paths", () => {
    ...
    try {
      const policy = makeBasePolicy({
        fs: { allowRead: [tmpDir], allowWrite: [], denyPatterns: ["/etc/passwd"] },
      });
      const capSet = caps.buildCapabilitySet(policy, makeCtx({ platform: "linux", cwd: tmpDir }));
  ...
  it("Linux: no platformRules even when denyPatterns contains simple paths", () => {
    ...
    const capSet = caps.buildCapabilitySet(policy, makeCtx({ platform: "linux" }));
    expect(capSet.platformRules).toEqual([]);
  });
  ...
  it("macOS: optional platformRule entries for simple deny paths", () => {
    ...
    const capSet = caps.buildCapabilitySet(policy, makeCtx({ platform: "darwin" }));
    expect(capSet.platformRules.some((r) => r.includes("/etc/passwd"))).toBe(true);
    expect(capSet.platformRules.every((r) => !r.includes("**/*.pem"))).toBe(true);
  });
...
describe("escapeSeatbeltString", () => {
  it("returns plain strings unchanged", () => {
    ...
    expect(escapeSeatbeltString("/home/user/some path/file.txt")).toBe("/home/user/some path/file.txt");
  });
...
describe("buildCapabilitySet — Seatbelt escaping", () => {
  ...
  it('macOS: deny path with double-quote is escaped in platformRule', () => {
    ...
    const capSet = caps.buildCapabilitySet(policy, makeCtx({ platform: "darwin" }));
    expect(capSet.platformRules.some((r) => r.includes('/etc/pa\\"sswd'))).toBe(true);
  });
  ...
  it("macOS: deny path with control character throws at manifest-build time", () => {
    ...
    expect(() => caps.buildCapabilitySet(policy, makeCtx({ platform: "darwin" }))).toThrow(
      "control characters"
    );
  });
  ...
  it("Linux: deny path with control character does NOT throw (no Seatbelt on Linux)", () => {
    ...
    expect(() => caps.buildCapabilitySet(policy, makeCtx({ platform: "linux" }))).not.toThrow();
  });
});
...
describe("buildManifest — prefix push-down", () => {
  ...
  it("darwin: anchored default-style patterns after expansion produce non-empty deny set", () => {
    ...
    const manifest = caps.buildManifest(policy, makeCtx({ platform: "darwin", cwd }));
    const denyAccess = manifest.filesystem?.deny?.access ?? [];
...
describe("buildCapabilitySet — prefix push-down", () => {
  ...
  it("darwin: platformRules contain path-prefix rules for anchored patterns", () => {
    ...
    const capSet = caps.buildCapabilitySet(policy, makeCtx({ platform: "darwin" }));
    expect(capSet.platformRules.some((r) => r.includes("/home/user/.ssh/"))).toBe(true);
    expect(capSet.platformRules.every((r) => !r.includes("**/.env"))).toBe(true);
  });
