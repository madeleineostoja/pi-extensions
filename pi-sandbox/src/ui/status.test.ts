...
import { createSlashCommands, type SubcommandContext } from "../slash/commands.js";
...
function makeStatusUI(): { ui: StatusUI; calls: Array<{ key: string; text: string | undefined }> } {
...
function makeSubcommandContext(policyManager: PolicyManager): SubcommandContext {
  return {
    ui: { notify: () => {} },
    policyManager,
    cwd: "/tmp",
...
describe("renderStatus — pure renderer", () => {
  ...
  it("network mode 'always' + hasUI=true → normal (N hosts)", () => {
    const result = renderStatus({ ...baseState, networkMode: "always", hasUI: true });
    expect(result).toBe("🔒 sandbox · 12 hosts · 3 writable");
  });

  it("network mode 'always' + hasUI=false → normal (N hosts)", () => {
    const result = renderStatus({ ...baseState, networkMode: "always", hasUI: false });
    expect(result).toBe("🔒 sandbox · 12 hosts · 3 writable");
  });
  ...
  it("network off takes precedence over non-interactive-only + hasUI", () => {
    const result = renderStatus({ ...baseState, networkOff: true, hasUI: true });
    expect(result).toBe("🔒 sandbox · network: ⚠ off");
  });
  ...
  it("disabled takes precedence over in-process only", () => {
    const result = renderStatus({ ...baseState, enabled: false, inProcessOnly: true });
    expect(result).toBe("⚠ sandbox: off");
  });
  ...
  it("network mode 'off' + hasUI=true → network: off (config)", () => {
    const result = renderStatus({ ...baseState, networkMode: "off", hasUI: true });
    expect(result).toBe("🔒 sandbox · network: off (config)");
  });

  it("session networkOff takes precedence over mode 'off'", () => {
    const result = renderStatus({ ...baseState, networkMode: "off", networkOff: true });
    expect(result).toBe("🔒 sandbox · network: ⚠ off");
  });
});
...
describe("subscribeStatus — integration", () => {
  ...
  it("calls setStatus immediately on subscribe", () => {
    const policy = makePolicy({ network: { mode: "always", allow: ["a.com", "b.com"] } });
  ...
  it("initial normal state with hasUI=false → N hosts · M writable", () => {
    const policy = makePolicy({
      ...
      fs: { allowRead: ["/tmp"], allowWrite: ["/tmp", "/var"], denyPatterns: [] },
    });
    const policyManager = makePolicyManager(policy);
    const { ui, calls } = makeStatusUI();
    const { onSessionMutation } = makeSessionMutationSubscriber();
  ...
  it("/sandbox network off → flips to network: ⚠ off", () => {
    const policy = makePolicy({ network: { mode: "always", allow: ["a.com"] } });
    const policyManager = makePolicyManager(policy);
    const ctx = makeSubcommandContext(policyManager);
    const { ui, calls } = makeStatusUI();
    const { onSessionMutation, triggerMutation } = makeSessionMutationSubscriber();
  ...
  it("non-interactive-only + hasUI=true → network: off (interactive)", () => {
    const policy = makePolicy({ network: { mode: "non-interactive-only", allow: [] } });
    const policyManager = makePolicyManager(policy);
    const { ui, calls } = makeStatusUI();
    const { onSessionMutation } = makeSessionMutationSubscriber();
  ...
  it("policy reload updates host count", () => {
    ...
    let currentPolicy = makePolicy({ network: { mode: "always", allow: ["a.com"] } });
    ...
    currentPolicy = makePolicy({ network: { mode: "always", allow: ["a.com", "b.com", "c.com"] } });
