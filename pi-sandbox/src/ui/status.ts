...
export function subscribeStatus(opts: StatusSubscribeOptions): () => void {
  const { policyManager, getSessionState, hasUI, ui, onSessionMutation, inProcessOnly } = opts;
  ...
  function buildState(): StatusState {
    ...
    return {
      ...
      networkMode: policy.network.mode,
      hasUI,
      allowedHostCount: policy.network.allow.length + session.sessionAllowedHosts.size,
