...
function makeUI(): { ui: SubcommandContext["ui"]; messages: Array<{ text: string; level?: string }> } {
...
function makeCtx(overrides: Partial<SubcommandContext> = {}): SubcommandContext {
...
describe("session state — allow", () => {
  ...
  beforeEach(() => { cmds = createSlashCommands(createAuditPipeline()); });
...
describe("session state — revoke", () => {
  ...
  it("removes a host from session grants", () => {
    ...
    expect(cmds.getSessionState().sessionAllowedHosts.has("example.com")).toBe(false);
  });
...
describe("audit event emission", () => {
  ...
  it("network off emits sandbox:audit", () => {
    ...
    cmds.handleNetworkOff(ctx);
    expect(emitted.filter((e) => e.event === "sandbox:audit").length).toBeGreaterThan(0);
  });
  ...
  it("every mutation also emits sandbox:policy-changed", () => {
    ...
    const ctx = makeCtx({ ui, events });
    cmds.handleAllow(ctx, ["x.com"], false);
    expect(emitted.filter((e) => e.event === "sandbox:policy-changed").length).toBeGreaterThan(0);
  });
});
...
function completionValues(items: ReturnType<typeof getArgumentCompletions>): string[] {
  if (!items) return [];
...
describe("getArgumentCompletions — tab completion", () => {
  ...
  it("'allow ' → recently-blocked hosts", () => {
    ...
    const pm = makePolicyManager(makePolicy());
    const values = completionValues(getArgumentCompletions("allow ", pm, undefined, pipeline.getRecentBlockedHosts));
  ...
  it("'allow ' → does not suggest already-typed hosts", () => {
    const { events } = makeEvents();
    const pipeline = createAuditPipeline();
    pipeline.recordAudit({ kind: "network", decision: "blocked", host: "already.com" }, { events });
    const pm = makePolicyManager(makePolicy());
    const values = completionValues(getArgumentCompletions("allow already.com ", pm, undefined, pipeline.getRecentBlockedHosts));
...
describe("dispatch — subcommand routing", () => {
  ...
  it("'allow github.com' → session grant", () => {
    ...
    expect(cmds.getSessionState().sessionAllowedHosts.has("github.com")).toBe(true);
  });
...
describe("handleWhy", () => {
  ...
  it("path in fs.allowRead → reports would be allowed", async () => {
    const { ui, messages } = makeUI();
    const policy = makePolicy({ fs: { allowRead: ["/tmp"], allowWrite: [], denyPatterns: [] } });
  ...
  it("path matching denyPattern → reports blocked by denyPattern", async () => {
    ...
    const ctx = makeCtx({ ui, policyManager: makePolicyManager(policy), cwd: "/tmp" });
  ...
  it("path not in fs.allowRead → reports blocked", async () => {
    const { ui, messages } = makeUI();
    const policy = makePolicy({ fs: { allowRead: ["/allowed"], allowWrite: [], denyPatterns: [] } });
    const ctx = makeCtx({ ui, policyManager: makePolicyManager(policy) });
  ...
  it("handleWhy and decideFsAccess agree on allowed path", async () => {
    ...
    const decision = await decideFsAccess(target, "read", policy, { cwd: os.tmpdir() });
  ...
  it("handleWhy and decideFsAccess agree on denyPattern block", async () => {
    ...
    await cmds.handleWhy(ctx, target);
    const decision = await decideFsAccess(target, "read", policy, { cwd: "/tmp" });
...
describe("per-host audit emission", () => {
  ...
  it("allow with multiple hosts emits one audit event per host", () => {
    ...
    const hosts = auditEvents.map((e) => (e.payload as Record<string, unknown>).host);
  ...
  it("network off audit event includes rule=network-off", () => {
    ...
    expect((auditEvents[0].payload as Record<string, unknown>).rule).toBe("network-off");
  });
  ...
  it("sandbox off audit event includes rule=sandbox-off", () => {
    ...
    const auditEvents = emitted.filter((e) => e.event === "sandbox:audit");
    expect(auditEvents).toHaveLength(1);
    expect((auditEvents[0].payload as Record<string, unknown>).rule).toBe("sandbox-off");
  });
});
