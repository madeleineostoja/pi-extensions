import type { Policy, FsPolicy, NetworkPolicy, AuditPolicy, EnforcementPolicy } from "./defaults.js";
...
const KNOWN_TOP_LEVEL = new Set(["enabled", "fs", "network", "audit", "enforcement"]);
...
function rejectUnknownKeys(obj: Record<string, unknown>, known: Set<string>, path: string): void {
...
function validateEnforcement(raw: unknown): Partial<EnforcementPolicy> {
  ...
  if (obj.requireKernelSandbox !== undefined) {
    if (typeof obj.requireKernelSandbox !== "boolean") {
      throw new PolicyValidationError('"enforcement.requireKernelSandbox" must be a boolean');
    }
 ...
 */
export function validatePolicy(raw: unknown): PartialPolicy & Pick<Partial<Policy>, "enabled"> {
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
