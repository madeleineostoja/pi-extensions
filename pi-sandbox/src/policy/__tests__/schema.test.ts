...
import { createPolicyManager } from "../load.js";
...
describe("validatePolicy", () => {
  ...
  it("rejects unknown top-level fields", () => {
    expect(() => validatePolicy({ unknownField: true })).toThrow(PolicyValidationError);
    expect(() => validatePolicy({ unknownField: true })).toThrow(/Unknown field/);
  });

  it("rejects unknown fs fields", () => {
    expect(() => validatePolicy({ fs: { allowRead: [], extra: true } })).toThrow(
  ...
  it("rejects wrong type for enabled", () => {
    expect(() => validatePolicy({ enabled: "yes" })).toThrow(PolicyValidationError);
    expect(() => validatePolicy({ enabled: 1 })).toThrow(PolicyValidationError);
  });

  it("rejects wrong type for network.mode", () => {
    expect(() =>
      validatePolicy({ network: { mode: "invalid-mode" } })
    ).toThrow(PolicyValidationError);
  });
  ...
  it("rejects non-string entries in string arrays", () => {
    expect(() =>
      validatePolicy({ fs: { allowRead: ["/good", 42] } })
    ).toThrow(PolicyValidationError);
  });

  it("rejects wrong type for audit.log", () => {
    expect(() => validatePolicy({ audit: { log: "true" } })).toThrow(PolicyValidationError);
  });

  it("rejects wrong type for audit.logFile", () => {
    expect(() => validatePolicy({ audit: { logFile: 99 } })).toThrow(PolicyValidationError);
  });
  ...
  it("rejects non-object network", () => {
    expect(() => validatePolicy({ network: [1, 2] })).toThrow(PolicyValidationError);
  });

  it("rejects non-object audit", () => {
    expect(() => validatePolicy({ audit: null })).toThrow(PolicyValidationError);
  });

  it("accepts valid hostname entries in network.allow", () => {
    const result = validatePolicy({
      network: { allow: ["example.com", "api.example.com", "*.example.com", "*.github.com", "github.com"] },
    });
  ...
  it("rejects malformed hostnames in network.allow", () => {
    ...
    expect(() =>
      validatePolicy({ network: { allow: [""] } })
    ).toThrow(PolicyValidationError);
    expect(() =>
      validatePolicy({ network: { allow: ["host/path"] } })
    ).toThrow(PolicyValidationError);
  });
});
...
describe("loadPolicy — darwin unanchored denyPattern warning", () => {
  ...
  it("darwin + unanchored pattern → ui.notify warning fires once per unanchored pattern", () => {
    ...
    const warnCalls = (mockUi.notify as ReturnType<typeof vi.fn>).mock.calls.filter(
      ([msg, level]) => level === "warning" && (msg as string).includes("no literal prefix")
    );
