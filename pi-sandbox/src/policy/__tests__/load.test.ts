...
import { createPolicyManager, expandPaths, expandPathsInPolicy } from "../load.js";
...
describe("expandPaths", () => {
  ...
  it("expands ~ to the home directory", () => {
    expect(expandPaths("~/.cache", "/project", "/home/user")).toBe("/home/user/.cache");
  ...
  it("leaves an unset env var unexpanded", () => {
    ...
    expect(expandPaths("$DOES_NOT_EXIST/sub", "/project")).toBe("$DOES_NOT_EXIST/sub");
  });
...
describe("expandPathsInPolicy", () => {
  ...
  it("regression: expands audit.logFile as before", () => {
    const result = expandPathsInPolicy(BASE_POLICY, "/wd", "/home/user");
    expect(result.audit.logFile).toBe("/home/user/.pi/agent/logs/sandbox-audit.jsonl");
  });
});
...
describe("loadPolicy", () => {
  ...
  it("does not warn when network.mode is 'always' and allow is non-empty", () => {
    const { loadPolicy } = createPolicyManager();
    const globalPath = path.join(tmpHome, ".pi", "agent", "sandbox.json");
    writeJson(globalPath, { network: { mode: "always", allow: ["example.com"] } });
