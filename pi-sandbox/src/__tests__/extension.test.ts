...
vi.mock("../enforcement/subprocess.js", () => ({
  initSubprocessSandbox: vi.fn().mockReturnValue({
    ...
    nonoPath: null,
    spawnHook: vi.fn((input: unknown) => input),
    userBashHandler: vi.fn((event: unknown) => ({ operations: [event] })),
  }),
}));
...
function makePi(overrides: Partial<Record<string, unknown>> = {}): ExtensionAPI {
  ...
  return {
    on: vi.fn((event: string, handler: (event: unknown, ctx: ExtensionContext) => unknown) => {
      ...
      if (existing) {
        handlers.set(event, (e, c) => { existing(e, c); return handler(e, c); });
      } else {
...
function makeCtx(overrides: Partial<Record<string, unknown>> = {}): ExtensionContext {
  const setStatusFn = vi.fn();
...
describe("sandboxExtension — wiring", () => {
  it("registers pi.on('tool_call', ...) after session_start fires", () => {
    ...
    const onCalls = (pi.on as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const toolCallRegistrations = onCalls.filter((args) => args[0] === "tool_call");
  ...
  it("registers pi.registerCommand('sandbox', ...) after session_start fires", () => {
    ...
    expect(pi.registerCommand).toHaveBeenCalledWith("sandbox", expect.objectContaining({
  ...
  it("calls ctx.ui.setStatus at least once with a non-undefined initial render", () => {
    ...
    const setStatusCalls = (ctx.ui.setStatus as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    expect(setStatusCalls.length).toBeGreaterThanOrEqual(1);
    const sandboxStatusCalls = setStatusCalls.filter((args) => args[0] === "sandbox");
...
describe("sandboxExtension — session mutation via command handler", () => {
  it("after /sandbox off, tool_call is allowed and status is re-rendered", async () => {
    ...
    const registerCalls = (pi.registerCommand as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const sandboxCmd = registerCalls.find((args) => args[0] === "sandbox");
    expect(sandboxCmd).toBeDefined();
    const cmdOptions = (sandboxCmd as unknown[])[1] as { handler: (args: string, ctx: ExtensionContext) => Promise<void> };
    ...
    const lastStatusCall = setStatusSpy.mock.calls[setStatusSpy.mock.calls.length - 1];
...
describe("sandboxExtension — double invocation", () => {
  it("second session_start on the same pi instance is a no-op and emits a warning", () => {
    ...
    sandboxExtension(pi);
    fireSessionStart(pi, ctx);
    const registerCallsAfterFirst = (pi.registerCommand as ReturnType<typeof vi.fn>).mock.calls.length as number;

    // Fire session_start again (simulates reload/double-load scenario)
    fireSessionStart(pi, ctx);
    const registerCallsAfterSecond = (pi.registerCommand as ReturnType<typeof vi.fn>).mock.calls.length as number;

    // No additional registrations on second session_start
    expect(registerCallsAfterSecond).toBe(registerCallsAfterFirst);

    // Warning was emitted
    const notifyCalls = (ctx.ui.notify as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const warnings = notifyCalls.filter((args) => args[1] === "warning");
    const doubleCallWarning = warnings.find((args) => typeof args[0] === "string" && (args[0] as string).includes("twice"));
...
describe("sandboxExtension — strict-mode refusal", () => {
  ...
  it("when requireKernelSandbox=true and binary unavailable, emits error notify and does not register tool_call", () => {
    tmpDir = fs.mkdirSync(path.join(os.tmpdir(), `pi-sandbox-strict-test-${Date.now()}`), { recursive: true }) as string;
    const piDir = path.join(tmpDir, ".pi");
    fs.mkdirSync(piDir, { recursive: true });
    fs.writeFileSync(path.join(piDir, "sandbox.json"), JSON.stringify({ enforcement: { requireKernelSandbox: true } }), "utf8");
    ...
    const requireKernelError = errorCalls.find(
      (args) => typeof args[0] === "string" && (args[0] as string).includes("requireKernelSandbox")
    );
...
describe("sandboxExtension — two-instance isolation", () => {
  it("toggling /sandbox off on one instance does not affect the other instance's session state", async () => {
    ...
    const registerCalls1 = (pi1.registerCommand as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const sandboxCmd1 = registerCalls1.find((args) => args[0] === "sandbox");
    expect(sandboxCmd1).toBeDefined();
    const cmdHandler1 = ((sandboxCmd1 as unknown[])[1] as { handler: (args: string, ctx: ExtensionContext) => Promise<void> }).handler;

    // Get sandbox command handler for pi2
    const registerCalls2 = (pi2.registerCommand as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const sandboxCmd2 = registerCalls2.find((args) => args[0] === "sandbox");
    expect(sandboxCmd2).toBeDefined();
    const cmdHandler2 = ((sandboxCmd2 as unknown[])[1] as { handler: (args: string, ctx: ExtensionContext) => Promise<void> }).handler;
    ...
    const result2 = await fireToolCall(pi2, ctx2, event);
    ...
    const setStatusCalls2 = (ctx2.ui.setStatus as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const sandboxStatusCalls2 = setStatusCalls2.filter((args) => args[0] === "sandbox");
    const lastStatus2 = sandboxStatusCalls2[sandboxStatusCalls2.length - 1]?.[1] as string | undefined;
    ...
    const setStatusCalls2After = (ctx2.ui.setStatus as ReturnType<typeof vi.fn>).mock.calls as unknown[][];
    const sandboxStatusCalls2After = setStatusCalls2After.filter((args) => args[0] === "sandbox");
    const lastStatus2After = sandboxStatusCalls2After[sandboxStatusCalls2After.length - 1]?.[1] as string | undefined;
...
describe("sandboxExtension — policy load failure", () => {
  ...
  it("reports parse failure via ctx.ui.notify at error/warning level and still registers the tool_call handler", () => {
    tmpDir = fs.mkdirSync(path.join(os.tmpdir(), `pi-sandbox-test-${Date.now()}`), { recursive: true }) as string;
    const piDir = path.join(tmpDir, ".pi");
    fs.mkdirSync(piDir, { recursive: true });
    fs.writeFileSync(path.join(piDir, "sandbox.json"), "{ this is: not valid json {{{{", "utf8");
    ...
    const parseFailureCall = errorOrWarnCalls.find(
      (args) => typeof args[0] === "string" && (args[0] as string).toLowerCase().includes("invalid json")
    );
