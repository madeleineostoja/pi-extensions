...
Each finding is atomic and receives an orchestrator-owned ID. Initial review remains the complete reviewer pass; configure the reviewer with high thinking for the intended review rigor. Its blockers are proposals until an internal admission step classifies them as admitted, deferred, demoted, or rejected. Admission inherits the reviewer model/type but always uses low thinking and no repository tools; it is not a configurable adjudicator role. If admission is uncertain, unavailable, malformed, stale, or incomplete, affected proposals are admitted conservatively. Deferred concerns remain durable leads for the final overall review without becoming task blockers.

Anchored re-reviews assess every outstanding ID and can add only regressions caused by the latest candidate delta. Rework declarations account for each outstanding ID as addressed or not addressed, while the reviewer decides resolution. Semantic progress is a strict low-water mark: there is no total review cap, but two consecutive rounds that do not beat the best outstanding count stall the retained candidate. Provider and protocol failures remain separately bounded safety retries.
...
In a TUI session, pi-implement shows a status footer summarizing the current phase and a widget listing active subagents with their runtime stats (tokens, tool uses, compaction counts). It also streams `pi-implement` progress messages into the session as tasks start, finish, get reviewed, and land. Detailed status projects canonical review transitions: candidate A→B, admission dispositions, rework declarations, resolved/unresolved and newly admitted regression IDs, and previous/current/best outstanding counts. The compact footer and widget stay bounded and do not show finding text. These are pi-implement's own authoritative updates — the host agent stays idle while a run is in flight.
...
Open `/implement` and choose **Stop run** to halt local orchestration and request that active subagents stop. If a run is blocked, stopped, or stalled, choose **Inspect artifacts** to locate its run directory, task/overall candidates, outstanding finding IDs, and per-round evidence under `<repo>/.pi/implement/`. Every round retains its implementer/reviewer prompts and typed results, proposal/admission and rework/anchored-assessment evidence, candidate identity/diff, finding transition, validation evidence, runtime health/model metadata when available, and any discarded worker-delta bundle. Canonical review state remains authoritative for stage and progress; artifacts are durable supporting evidence for recovery and inspection.
