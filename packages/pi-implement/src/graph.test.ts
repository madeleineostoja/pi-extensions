...
describe("parseStrategyDecision legacy review fields", () => {
  it("ignores graph node review directives", () => {
    const graph = makeGraph([
      {
        ...makeNode({ id: "t1", planIndex: 1 }),
        review: { mode: "skip", reason: "docs only" },
      } as ReturnType<typeof makeNode> & { review: unknown },
    ]);
    const result = parseStrategyDecision(
      JSON.stringify({
        mode: "parallel",
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.graph?.nodes[0]).not.toHaveProperty("review");
    }
  });
});
