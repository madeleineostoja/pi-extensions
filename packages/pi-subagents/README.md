...
| Agent     | Use it for                                                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `General` | Bounded work that materially benefits from separate context, parallel execution, or a different model. Prefer research or synthesis; ordinary implementation stays with the primary agent. |
...
While the `Agent` tool is active, it injects routing guidelines into the parent prompt. These direct non-trivial codebase discovery to `Explore` and encourage `Review` as an independent second pass for large, risky, or multi-file changes. They reserve `General` for bounded work that materially benefits from separate context, parallel execution, or a different model, with a preference for independent research or synthesis. The primary agent should retain ordinary implementation, debugging, and routine test execution; implementation should go to `General` only when explicitly requested or safely isolated with clear, non-overlapping ownership. A fresh session whose primary task is already a review should review directly rather than delegate without useful context isolation.
