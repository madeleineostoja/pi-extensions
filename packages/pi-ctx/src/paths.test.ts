...
describe("normalizePath", () => {
  ...
  it("resolves a ../ path against cwd", () => {
    expect(normalizePath("../sibling/foo.ts", "/repo/sub")).toBe(
      "/repo/sibling/foo.ts",
    );
  });
  ...
  it("expands ~/ to the home directory", () => {
    expect(normalizePath("~/projects/foo.ts", "/repo")).toBe(
      `${homedir()}/projects/foo.ts`,
    );
  });
...
describe("extractFilePath", () => {
  ...
  it("returns file_path when path is absent (Pi accepts both)", () => {
    expect(extractFilePath("read", { file_path: "src/foo.ts" })).toBe(
      "src/foo.ts",
    );
  });

  it("prefers path over file_path when both are present", () => {
    expect(extractFilePath("read", { path: "a.ts", file_path: "b.ts" })).toBe(
      "a.ts",
    );
  });

  it("returns file_path for edit tool", () => {
    expect(extractFilePath("edit", { file_path: "src/foo.ts" })).toBe(
      "src/foo.ts",
    );
  });

  it("returns file_path for write tool", () => {
    expect(extractFilePath("write", { file_path: "src/foo.ts" })).toBe(
      "src/foo.ts",
    );
  });
...
describe("normalizePath + extractFilePath combined", () => {
  it("end-to-end: extracts and normalizes a file path from a read input", () => {
    ...
    const cwd = "/repo";
    expect(normalizePath(extractFilePath("read", input), cwd)).toBe(
      "/repo/src/foo.ts",
    );
  });
