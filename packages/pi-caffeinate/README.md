...
Starts on `session_start`, stops on `session_shutdown`. The child watches pi's PID and exits on its own when pi dies, so the inhibitor is released even if pi is killed with `SIGKILL` and our shutdown handlers never run.
