...
const RecallParams = Type.Object({
	...
	lines: Type.Optional(
		Type.String({
			description: 'Optional 1-indexed line range like "10-20" or single line "5"',
		}),
	),
});
...
export function registerRecallTool(pi: ExtensionAPI, onRecall?: (toolName: string) => void): void {
	pi.registerTool({
		name: "ctx_recall",
		label: "ctx_recall",
		description:
		...
		promptSnippet: 'ctx_recall("toolCallId") — retrieve a tool result that was replaced with an elision stub',
		promptGuidelines: [
		...
		async execute(
			...
			const recalledToolName = entry?.type === "message"
