...
export function isEligibleForElision(turnDistanceFromEnd: number, tokenCount: number, staleTurns = DEFAULTS.staleTurns, minTokens = DEFAULTS.minTokens): boolean {
...
export function extractPreview(content: ContentBlock[]): string | null {
...
export function estimateContentTokens(content: ContentBlock[]): number {
	let chars = 0;
...
export function userTurnsAfterEachPosition(messages: AgentMsg[]): number[] {
	const distances: number[] = new Array(messages.length).fill(0);
	let userCount = 0;
...
export function userTurnsUpToEachPosition(messages: AgentMsg[]): number[] {
	const counts: number[] = new Array(messages.length).fill(0);
	let userCount = 0;
...
function isToolResult(msg: AgentMsg): msg is AgentMsg & ToolResultLike {
		...
		if (msg.role === "assistant" && Array.isArray(msg.content)) {
			for (const block of msg.content) {
				...
				if (b.type === "toolCall" && typeof b.id === "string" && typeof b.name === "string") {
					toolCallInfoMap.set(b.id, { name: b.name, input: b.arguments });
				}
			}
		}
...
function buildDuplicateReadMap(
	...
	for (let i = 0; i < messages.length; i++) {
		...
		entries.push({ toolCallId: msg.toolCallId, normalizedPath: normalized, userTurnIndex: userTurnCounts[i] });
	...
	for (const entries of groups.values()) {
		...
		for (const entry of entries.slice(0, -1)) {
			result.set(entry.toolCallId, { normalizedPath: entry.normalizedPath, keptUserTurnIndex: kept.userTurnIndex });
		}
	}
...
export function makeContextHook(config: Config, onElisionPass?: (result: ElisionPassResult) => void) {
	return function handleContext(
		...
		if (config.supersededReadsEnabled || config.duplicateReadsEnabled) {
			({ toolCallInfoMap, supersededPaths } = buildSupersededMaps(messages, cwd));
		}
		...
		for (let i = 0; i < messages.length; i++) {
			...
			const isGenericStale = !msg.isError && isEligibleForElision(distance, tokenCount, config.staleTurns, config.minTokens);

			let supersededNormalizedPath: string | null = null;
			if (config.supersededReadsEnabled && msg.toolName === "read" && toolCallInfoMap && supersededPaths) {
				const callInfo = toolCallInfoMap.get(msg.toolCallId);
				if (callInfo) {
					const rawPath = extractFilePath("read", callInfo.input);
					const normalized = normalizePath(rawPath, cwd);
			...
			if (config.duplicateReadsEnabled && msg.toolName === "read" && !msg.isError && duplicateReadMap) {
				duplicateInfo = duplicateReadMap.get(msg.toolCallId) ?? null;
			}

			if (!isGenericStale && supersededNormalizedPath === null && duplicateInfo === null) {
				result.push(msg);
				continue;
			}
			...
			const elided: AgentMsg = {
				role: "toolResult",
				toolCallId: msg.toolCallId,
				toolName: msg.toolName,
				content: [{ type: "text", text: stub } as TextBlock],
