...
type Notifier = (msg: string, level: "warning" | "info" | "error") => void;
...
export function loadConfig(
	...
	const configPath = join(home, ".pi", "agent", "extensions", "pi-ctx", "config.json");
	...
	try {
		...
		notify?.("pi-ctx: config file contains malformed JSON; using defaults", "warning");
		return defaultConfig();
	}

	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
		notify?.("pi-ctx: config file must be a JSON object; using defaults", "warning");
		return defaultConfig();
	}
	...
	const numKeys: Array<"staleTurns" | "minTokens"> = ["staleTurns", "minTokens"];
