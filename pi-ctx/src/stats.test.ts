...
describe("createStatsStore", () => {
	...
	it("accumulates tokensElidedCumulative across elision passes with distinct ids", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "id-1", tokenCount: 100, toolName: "read" }] });
		store.onElisionPass({ entries: [{ toolCallId: "id-2", tokenCount: 200, toolName: "read" }] });
	...
	it("two distinct ids across two passes sum to both sizes", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "id-a", tokenCount: 400, toolName: "bash" }] });
		store.onElisionPass({ entries: [{ toolCallId: "id-b", tokenCount: 600, toolName: "bash" }] });
	...
	it("elidedCountLatest reflects only the most recent pass, not cumulative", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "id-1", tokenCount: 500, toolName: "read" }, { toolCallId: "id-2", tokenCount: 100, toolName: "read" }, { toolCallId: "id-3", tokenCount: 200, toolName: "bash" }] });
		store.onElisionPass({ entries: [{ toolCallId: "id-4", tokenCount: 100, toolName: "bash" }] });
		expect(store.snapshot().elidedCountLatest).toBe(1);
	});

	it("elidedCountLatest updates on each pass", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "a", tokenCount: 10, toolName: "read" }, { toolCallId: "b", tokenCount: 20, toolName: "read" }, { toolCallId: "c", tokenCount: 30, toolName: "bash" }, { toolCallId: "d", tokenCount: 40, toolName: "bash" }, { toolCallId: "e", tokenCount: 50, toolName: "bash" }] });
		expect(store.snapshot().elidedCountLatest).toBe(5);
		store.onElisionPass({ entries: [{ toolCallId: "f", tokenCount: 10, toolName: "read" }, { toolCallId: "g", tokenCount: 20, toolName: "bash" }] });
	...
	it("reset returns all counters to zero", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "id-1", tokenCount: 2048, toolName: "read" }, { toolCallId: "id-2", tokenCount: 100, toolName: "bash" }, { toolCallId: "id-3", tokenCount: 200, toolName: "bash" }, { toolCallId: "id-4", tokenCount: 300, toolName: "read" }] });
		store.onRecall("read");
		store.reset();
		expect(store.snapshot()).toEqual({
			tokensElidedCumulative: 0,
	...
	it("snapshot is a value copy that does not change when store mutates", () => {
		...
		store.onElisionPass({ entries: [{ toolCallId: "id-1", tokenCount: 512, toolName: "read" }] });
		expect(snap1.tokensElidedCumulative).toBe(0);
	});

	it("per-tool bytes accumulate correctly across multiple passes", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "r1", tokenCount: 10000, toolName: "read" }, { toolCallId: "r2", tokenCount: 14000, toolName: "read" }] });
		store.onElisionPass({ entries: [{ toolCallId: "b1", tokenCount: 10000, toolName: "bash" }] });
	...
	it("dedup-by-toolCallId carries over to per-tool counts", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "r1", tokenCount: 5000, toolName: "read" }] });
		store.onElisionPass({ entries: [{ toolCallId: "r1", tokenCount: 5000, toolName: "read" }] });
		const snap = store.snapshot();
		const readTool = snap.byTool.find((t) => t.toolName === "read");
	...
	it("per-tool recall counter increments only for the tool of the recalled result", () => {
		const store = createStatsStore();
		store.onElisionPass({ entries: [{ toolCallId: "r1", tokenCount: 1000, toolName: "read" }, { toolCallId: "b1", tokenCount: 500, toolName: "bash" }] });
		store.onRecall("read");
		const snap = store.snapshot();
		const readTool = snap.byTool.find((t) => t.toolName === "read");
		const bashTool = snap.byTool.find((t) => t.toolName === "bash");
...
describe("formatStats", () => {
	...
	it("first three lines are preserved verbatim with elisions present", () => {
		...
		expect(lines[0]).toBe(`tokens elided (cumulative): ${formatTokenCount(34000)}`);
