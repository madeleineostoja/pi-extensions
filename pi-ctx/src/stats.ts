...
export function createStatsStore(): StatsStore {
	const elidedById = new Map<string, { tokenCount: number; toolName: string }>();
	...
	return {
		...
		snapshot(): StatsSnapshot {
			...
			for (const { tokenCount, toolName } of elidedById.values()) {
				...
				toolTokensMap.set(toolName, (toolTokensMap.get(toolName) ?? 0) + tokenCount);
...
export function formatStats(snapshot: StatsSnapshot): string {
	...
	if (snapshot.byTool.length > 0) {
		...
		for (const { toolName, tokens, entries, recalls } of snapshot.byTool) {
			lines.push(`  ${toolName}   ${formatTokenCount(tokens)}  (${plural(entries, "entry", "entries")}, ${plural(recalls, "recall", "recalls")})`);
		}
	}
