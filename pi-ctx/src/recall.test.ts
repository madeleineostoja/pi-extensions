...
async function executeRecall(
	...
	let capturedExecute: ((toolCallId: string, params: any, signal: any, onUpdate: any, ctx: any) => Promise<any>) | null = null;
...
describe("ctx_recall execute", () => {
	it("returns original content for a known id", async () => {
		...
		expect(result.content).toEqual([{ type: "text", text: "hello world content" }]);
	});

	it("returns isError true with message for unknown id", async () => {
		const entries = [makeSessionEntry("call-abc", "content")];
		const result = await executeRecall(entries as any, { id: "does_not_exist" });
		expect(result.isError).toBe(true);
		expect(result.content[0].text).toMatch(/ctx_recall: no tool result with id=does_not_exist/);
	});

	it("slices lines 10-20 from single text block", async () => {
		const lines = Array.from({ length: 30 }, (_, i) => `line ${i + 1}`);
		const entries = [makeSessionEntry("call-lines", lines.join("\n"))];
		const result = await executeRecall(entries as any, { id: "call-lines", lines: "10-20" });
		expect(result.isError).toBeFalsy();
	...
	it("slices single line using '5' format", async () => {
		...
		const result = await executeRecall(entries as any, { id: "call-single-line", lines: "5" });
	...
	it("returns empty string for out-of-range lines", async () => {
		...
		const result = await executeRecall(entries as any, { id: "call-oor", lines: "9999-10000" });
		expect(result.isError).toBeFalsy();
		expect(result.content[0].text).toBe("");
	});

	it("returns isError true when lines is provided against multiple text blocks", async () => {
		const entries = [makeMultiBlockEntry("call-multi", ["block one", "block two"])];
		const result = await executeRecall(entries as any, { id: "call-multi", lines: "1-1" });
		expect(result.isError).toBe(true);
	...
	it("returns isError true when lines is provided against image-bearing content", async () => {
		...
		const result = await executeRecall(entries as any, { id: "call-img", lines: "1" });
		expect(result.isError).toBe(true);
	...
	it("returns isError true for invalid lines argument", async () => {
		...
		const result = await executeRecall(entries as any, { id: "call-abc", lines: "abc" });
		expect(result.isError).toBe(true);
	...
	it("returns compaction error when compactionSummary entry exists and id is not found", async () => {
		const entries = [makeCompactionEntry(), makeSessionEntry("other-id", "other content")];
	...
	it("returns result normally when both compactionSummary and a matching toolResult exist", async () => {
		const entries = [makeCompactionEntry(), makeSessionEntry("call-exists", "found content")];
