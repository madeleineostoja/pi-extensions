...
describe("isEligibleForElision", () => {
	it("returns true when both thresholds are met exactly", () => {
		expect(isEligibleForElision(DEFAULTS.staleTurns, DEFAULTS.minTokens)).toBe(true);
	});

	it("returns true when both thresholds are exceeded", () => {
		expect(isEligibleForElision(DEFAULTS.staleTurns + 2, DEFAULTS.minTokens + 50)).toBe(true);
	});

	it("returns false when turn distance is below threshold", () => {
		expect(isEligibleForElision(DEFAULTS.staleTurns - 1, DEFAULTS.minTokens + 50)).toBe(false);
	});

	it("returns false when token count is below threshold", () => {
		expect(isEligibleForElision(DEFAULTS.staleTurns + 1, DEFAULTS.minTokens - 1)).toBe(false);
	});
...
describe("formatStub", () => {
	it("formats stub with sub-1K tokens", () => {
		const result = formatStub({ toolName: "read", tokenCount: 500, toolCallId: "abc-123" });
		expect(result).toBe('[read result elided: 500 tokens. Call ctx_recall("abc-123") to retrieve.]');
	});

	it("formats stub with K tokens", () => {
		const result = formatStub({ toolName: "bash", tokenCount: 1000, toolCallId: "xyz-789" });
		expect(result).toBe('[bash result elided: 1K tokens. Call ctx_recall("xyz-789") to retrieve.]');
	});

	it("formats stub with fractional K tokens", () => {
		const result = formatStub({ toolName: "grep", tokenCount: 1500, toolCallId: "id-1" });
		expect(result).toBe('[grep result elided: 1.5K tokens. Call ctx_recall("id-1") to retrieve.]');
	});

	it("matches the stub regex pattern", () => {
		const result = formatStub({ toolName: "read", tokenCount: 300, toolCallId: "tool-call-id-abc" });
		expect(result).toMatch(/^\[\w+ result elided: (\d+(\.\d+)?K? tokens)\. Call ctx_recall\("[^"]+"\) to retrieve\.\]$/);
	});

	it("includes Preview segment when preview is provided", () => {
		const result = formatStub({ toolName: "read", tokenCount: 300, toolCallId: "id-prev", preview: "hello world" });
		expect(result).toBe('[read result elided: 300 tokens. Preview: "hello world". Call ctx_recall("id-prev") to retrieve.]');
	});

	it("omits Preview segment when preview is null", () => {
		const result = formatStub({ toolName: "read", tokenCount: 300, toolCallId: "id-null", preview: null });
		expect(result).toBe('[read result elided: 300 tokens. Call ctx_recall("id-null") to retrieve.]');
	});

	it("stub with preview starts with token count and ends with retrieve", () => {
		const result = formatStub({ toolName: "read", tokenCount: 300, toolCallId: "id-300", preview: "some preview…" });
		expect(result).toMatch(/^\[read result elided: 300 tokens\. Preview: ".*"\. Call ctx_recall\("id-300"\) to retrieve\.\]$/);
	});
});
...
describe("extractPreview", () => {
	...
	it("returns null for content with only image blocks", () => {
		const content = [{ type: "image" } as unknown as { type: string; text?: string }];
...
function makeUserMsg(): object {
	return { role: "user", content: [{ type: "text", text: "hello" }], timestamp: Date.now() };
}
...
describe("context hook", () => {
	...
	it("elides a large, old tool result", () => {
		...
		const elided = result.messages!.find((m: any) => m.role === "toolResult") as any;
		expect(elided.content[0].text).toMatch(/^\[read result elided:/);
		expect(elided.content[0].text).toMatch(/Call ctx_recall\("call-abc"\) to retrieve\.\]$/);
	});
	...
	it("does not elide a tool result that is too small", () => {
		...
		const messages = makeMessages(DEFAULTS.staleTurns + 1, toolResult);
		const result = hook({ type: "context", messages } as any, fakeCtx);
		const found = result.messages!.find((m: any) => m.role === "toolResult") as any;
	...
	it("elided message has exactly the canonical keys", () => {
		...
		expect(Object.keys(elided).sort()).toEqual(
			["content", "isError", "role", "timestamp", "toolCallId", "toolName"],
		);
	});
});
...
function makeAssistantMsg(toolCalls: Array<{ id: string; name: string; arguments: Record<string, unknown> }>): object {
	return {
		role: "assistant",
...
describe("superseded-read detection", () => {
	it("read followed by successful edit of same path → stubbed regardless of size/age", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "small" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]),
		...
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toMatch(/^\[read result elided \(superseded by later edit\/write of \/cwd\/src\/foo\.ts\):/);
		expect(readResult.content[0].text).toMatch(/Call ctx_recall\("read-1"\) to retrieve original\.\]$/);
	});
	...
	it("read followed by successful write of same path → stubbed", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: writeId, name: "write", arguments: { path: "src/foo.ts", content: "new" } }]),
			makeToolResultMsg({ toolCallId: writeId, toolName: "write", text: "written" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toMatch(/\[read result elided \(superseded by later edit\/write of/);
	});
	...
	it("read followed by edit of different path → NOT stubbed by superseded rule", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "x".repeat(10) }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/bar.ts", old_string: "a", new_string: "b" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
	...
	it("read followed by failed edit of same path (isError: true) → NOT stubbed", () => {
		...
		const messages: any[] = [
			...
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "error", isError: true }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
	...
	it("two reads of same path followed by one successful edit → both stubbed", () => {
		...
		const messages: any[] = [
			...
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "first read" }),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "second read" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
		const read2 = result.messages!.find((m: any) => m.toolCallId === readId2) as any;
	...
	it("read of ./src/foo.ts matched by edit of /cwd/src/foo.ts (same normalized path)", () => {
		...
		const messages: any[] = [
			...
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "/cwd/src/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toMatch(/superseded by later edit\/write of \/cwd\/src\/foo\.ts/);
	});
	...
	it("read of ../sibling/foo.ts matched by edit of /cwd/sibling/foo.ts (same resolved path)", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "../sibling/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "data" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "/cwd/sibling/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, ctxWithSubdir);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toMatch(/superseded by later edit\/write of \/cwd\/sibling\/foo\.ts/);
	});
	...
	it("stub starts with correct prefix and ends with retrieve original suffix", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "content here" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
		...
		expect(text).toMatch(/^\[read result elided \(superseded by later edit\/write of \/cwd\/src\/foo\.ts\):/);
		expect(text).toMatch(/Call ctx_recall\("read-fmt"\) to retrieve original\.\]$/);
	});
	...
	it("stub includes Preview segment when read had text content", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "hello world content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
		...
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toContain(' Preview: "hello world content".');
	});
	...
	it("edit before read, then another edit after read → read IS stubbed", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: editId1, name: "edit", arguments: { path: "src/foo.ts", old_string: "a", new_string: "b" } }]),
			makeToolResultMsg({ toolCallId: editId1, toolName: "edit", text: "ok" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "intermediate content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId2, name: "edit", arguments: { path: "src/foo.ts", old_string: "b", new_string: "c" } }]),
			makeToolResultMsg({ toolCallId: editId2, toolName: "edit", text: "ok" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toMatch(/superseded by later edit\/write/);
	});
	...
	it("edit of same path BEFORE the read does NOT supersede it", () => {
		...
		const messages: any[] = [
			...
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "fresh content" }),
		];
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
	...
	it("read that is BOTH superseded AND generic-stale eligible → superseded stub format wins", () => {
		...
		messages.push(makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]));
		messages.push(makeToolResultMsg({ toolCallId: readId, toolName: "read", text: bigText }));
		for (let k = 0; k < DEFAULTS.staleTurns; k++) messages.push(makeUserMsg());
		messages.push(makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]));
		messages.push(makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }));
		const hook = makeContextHook(supersededConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const readResult = result.messages!.find((m: any) => m.toolCallId === readId) as any;
		expect(readResult.content[0].text).toMatch(/superseded by later edit\/write/);
	...
	it("image-only read result with a later edit → stubbed but stub has no Preview segment", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", imageOnly: true }),
			makeUserMsg(),
			makeAssistantMsg([{ id: editId, name: "edit", arguments: { path: "src/foo.ts", old_string: "x", new_string: "y" } }]),
			makeToolResultMsg({ toolCallId: editId, toolName: "edit", text: "ok" }),
		];
...
describe("formatDuplicateStub", () => {
	it("formats stub with correct prefix and turn reference", () => {
		...
		expect(result).toBe('[read result elided (superseded by later read of /cwd/src/foo.ts at turn 3): 125 tokens. Call ctx_recall("read-1") to retrieve.]');
	});
...
function duplicateConfig() {
	return { ...defaultConfig(), duplicateReadsEnabled: true, supersededReadsEnabled: false };
}

function duplicateDisabledConfig() {
	return { ...defaultConfig(), duplicateReadsEnabled: false, supersededReadsEnabled: false };
}

function bothRulesConfig() {
	return { ...defaultConfig(), duplicateReadsEnabled: true, supersededReadsEnabled: true };
}
...
describe("duplicate-read detection", () => {
	it("two successful reads of same path → first stubbed, second intact", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "first content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "second content" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
		const read2 = result.messages!.find((m: any) => m.toolCallId === readId2) as any;
		expect(read1.content[0].text).toMatch(/^\[read result elided \(superseded by later read of/);
	...
	it("three successful reads of same path → first two stubbed, third intact", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "first" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "second" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId3, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId3, toolName: "read", text: "third" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
		const read2 = result.messages!.find((m: any) => m.toolCallId === readId2) as any;
		const read3 = result.messages!.find((m: any) => m.toolCallId === readId3) as any;
		expect(read1.content[0].text).toMatch(/^\[read result elided \(superseded by later read of/);
		expect(read2.content[0].text).toMatch(/^\[read result elided \(superseded by later read of/);
	...
	it("reads of src/foo.ts and src/bar.ts → neither is stubbed (different paths)", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "foo content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/bar.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "bar content" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
	...
	it("one successful + one failed read of same path → neither stubbed by duplicate rule", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "ok content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "error", isError: true }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
	...
	it("two successful + one failed read interleaved → first successful stubbed, last successful intact, failed intact", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "first ok" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "failed", isError: true }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId3, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId3, toolName: "read", text: "last ok" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
	...
	it("read of ./src/foo.ts and later read of /cwd/src/foo.ts (same normalized path) → first stubbed", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "./src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "relative content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "/cwd/src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "absolute content" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
	...
	it("stub starts with correct prefix including path and turn number", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "newer content" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
		expect(read1.content[0].text).toMatch(/^\[read result elided \(superseded by later read of \/cwd\/src\/foo\.ts at turn \d+\):/);
	});
	...
	it("stub includes Preview segment when read had text content", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "hello world content" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "newer" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
	...
	it("earliest read in a group of 4 is still stubbed (not just second-to-last)", () => {
		...
		const messages: any[] = [makeUserMsg()];
		for (const id of ids) {
			messages.push(makeAssistantMsg([{ id, name: "read", arguments: { path: "src/foo.ts" } }]));
			messages.push(makeToolResultMsg({ toolCallId: id, toolName: "read", text: `content-${id}` }));
		...
		for (const id of ids.slice(0, 3)) {
			...
			expect(read.content[0].text).toMatch(/^\[read result elided \(superseded by later read of/);
		}
		const last = result.messages!.find((m: any) => m.toolCallId === "quad-4") as any;
	...
	it("small read (< DEFAULTS.minTokens * 4) that is duplicated still gets stubbed (size threshold doesn't apply)", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId1, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId1, toolName: "read", text: "tiny" }),
			makeUserMsg(),
			makeAssistantMsg([{ id: readId2, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId2, toolName: "read", text: "also tiny" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read1 = result.messages!.find((m: any) => m.toolCallId === readId1) as any;
	...
	it("single read of a file is never stubbed by duplicate rule", () => {
		...
		const messages: any[] = [
			makeUserMsg(),
			makeAssistantMsg([{ id: readId, name: "read", arguments: { path: "src/foo.ts" } }]),
			makeToolResultMsg({ toolCallId: readId, toolName: "read", text: "only read" }),
		];
		const hook = makeContextHook(duplicateConfig());
		const result = hook({ type: "context", messages } as any, fakeCtxWithCwd);
		const read = result.messages!.find((m: any) => m.toolCallId === readId) as any;
