...
A [Pi coding agent](https://earendil.works) extension that keeps the context window small by eliding large, stale tool results and letting the LLM fetch them back on demand.
...
Run `/ctx-stats` in the Pi chat to see elision statistics for the current session:
